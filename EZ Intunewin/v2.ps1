...
function Work-PackageFolder {
    ...
    try {
        ...
        # Create temporary files for capturing process output
        $stdOutPath = Join-Path $env:TEMP "IntuneWinUtil_$($folderName)_stdout_$(Get-Date -Format 'yyyyMMdd_HHmmss').txt"
        $stdErrPath = Join-Path $env:TEMP "IntuneWinUtil_$($folderName)_stderr_$(Get-Date -Format 'yyyyMMdd_HHmmss').txt"

        $processInfo = Start-Process -FilePath $script:intuneWinUtilPath `
        ...
        if (Test-Path $stdErrPath) {
            ...
            if (-not [string]::IsNullOrWhiteSpace($errorOutput)) {
                Write-Log "  IntuneWinAppUtil stderr output:" -Level 'Verbose'
                Write-Log "  $errorOutput" -Level 'Verbose'
            }
        }
...
function Start-BulkProcessing {
    ...
    $totalSizeMB = 0
    $currentIndex = 0
    $totalPackages = $packageFolders.Count

    foreach ($packageFolder in $packageFolders) {
        $currentIndex++
        Write-Log "Processing package $currentIndex of $totalPackages" -Level 'Info'

        $result = Work-PackageFolder -PackageFolder $packageFolder
